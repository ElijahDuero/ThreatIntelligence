<script setup>
import { ref, computed, nextTick } from 'vue';
import { Link } from '@inertiajs/vue3';
import axios from 'axios';
import {
    Globe,
    Search,
    ExternalLink,
    Copy,
    Check,
    FolderPlus,
    Terminal,
    ShieldAlert,
    ShieldCheck,
    Loader2,
    RefreshCw,
    ArrowRight,
    X,
    Filter,
    Download,
    AlertTriangle,
    Server,
    CheckCircle2,
    Database,
    Shield,
    StopCircle,
    Activity,
    Compass,
    MapPin,
    Layers,
    FileText,
    ChevronDown,
    Wrench,
    Network,
} from 'lucide-vue-next';

import EvidenceBookmarkModal from '@/Components/EvidenceBookmarkModal.vue';
import { useEvidenceBookmark } from '@/Composables/useEvidenceBookmark';
import { downloadJson, downloadMarkdown } from '@/Utils/exportHelpers';

const props = defineProps({
    initialTarget: String,
    initialProbe: Object,
    initialFindings: Array,
    findingCategories: Array,
    categorizedTools: Object,
    investigations: Array,
    totalToolsCount: Number,
});

const targetInput = ref(props.initialTarget || '');
const currentTarget = ref(props.initialTarget || '');
const isProbing = ref(false);
const showHitsOnly = ref(false);
const selectedCategory = ref('all');
const toolSearchQuery = ref('');
const activeBranch = ref('all');
const showExportMenu = ref(false);

// Real-Time SSE Multi-Tool Streaming State
let eventSource = null;
const isStreaming = ref(false);
const discoveredFindings = ref(props.initialFindings ? [...props.initialFindings] : []);
const allStreamedResults = ref([]);
const streamProgress = ref({
    probed: 0,
    total: 7,
    found: props.initialFindings?.length || 0,
    percent: 0,
    last_tool: '',
    last_status: '',
    duration_ms: 0,
});

// Telemetry State
const probeData = ref(props.initialProbe || null);
const categorizedTools = ref(props.categorizedTools || {
    subdomains: [],
    host_port: [],
    web_dom: [],
    threat: [],
    dns: [],
});

// Bookmark Modal State
const { showBookmarkModal, bookmarkTarget, openBookmark } = useEvidenceBookmark(props.investigations);
const copiedField = ref(null);
const copiedUrlId = ref(null);

// 5 Unified Category Filter Definitions
const categoriesList = computed(() => props.findingCategories || [
    { id: 'all', label: 'All Findings' },
    { id: 'subdomains', label: 'Subdomains & Certs' },
    { id: 'ports', label: 'Host & Ports (Shodan)' },
    { id: 'web', label: 'Web & DOM (urlscan.io)' },
    { id: 'threat', label: 'Threat & Risk (ZoomEye)' },
    { id: 'dns', label: 'DNS Infrastructure' },
]);

// 5 Branch Definitions for Manual Tools Directory Sidebar
const branchDefinitions = [
    { id: 'subdomains', name: 'Subdomains & Certs', icon: Globe, count: 4 },
    { id: 'host_port', name: 'Host & Port Recon', icon: Server, count: 4 },
    { id: 'web_dom', name: 'Web & DOM Profiling', icon: Layers, count: 4 },
    { id: 'threat', name: 'Threat & Risk Intel', icon: ShieldAlert, count: 4 },
    { id: 'dns', name: 'DNS Diagnostics', icon: Network, count: 4 },
];

// Flat list of all framework catalog tools
const allToolsList = computed(() => {
    const list = [];
    Object.values(categorizedTools.value).forEach(branchTools => {
        if (Array.isArray(branchTools)) {
            list.push(...branchTools);
        }
    });
    return list;
});

// Category Counts for Findings
const categoryCounts = computed(() => {
    const list = showHitsOnly.value
        ? discoveredFindings.value.filter(f => f.status === 'found')
        : discoveredFindings.value;

    const counts = { all: list.length, subdomains: 0, ports: 0, web: 0, threat: 0, dns: 0 };
    list.forEach(item => {
        if (item.category && counts[item.category] !== undefined) {
            counts[item.category]++;
        }
    });
    return counts;
});

// 5 Forensic Bento Box Category Configurations
const branchCategoryConfig = [
    {
        id: 'subdomains',
        name: 'Subdomains & Certificate Transparency',
        shortName: 'Subdomains & Certs',
        description: 'SSL/TLS Certificate Transparency logs, SAN extensions, and passive subdomain discovery.',
        icon: Globe,
        accentColor: 'teal',
        accentBorder: 'border-teal-500/40 hover:border-teal-500/60',
        accentBg: 'bg-teal-500/10',
        accentText: 'text-teal-400',
        badgeClass: 'bg-teal-950/70 border-teal-500/40 text-teal-300',
        stripeGradient: 'from-teal-500/40 via-teal-500/10 to-transparent',
    },
    {
        id: 'ports',
        name: 'Host & Port Service Fingerprinting (Shodan)',
        shortName: 'Host & Ports',
        description: 'Live Shodan InternetDB inspection, active TCP ports, service banners, and hostnames.',
        icon: Server,
        accentColor: 'cyan',
        accentBorder: 'border-cyan-500/40 hover:border-cyan-500/60',
        accentBg: 'bg-cyan-500/10',
        accentText: 'text-cyan-400',
        badgeClass: 'bg-cyan-950/70 border-cyan-500/40 text-cyan-300',
        stripeGradient: 'from-cyan-500/40 via-cyan-500/10 to-transparent',
    },
    {
        id: 'web',
        name: 'Web Technology & DOM Telemetry (urlscan.io)',
        shortName: 'Web & DOM',
        description: 'urlscan.io automated profiling, HTTP response headers, web server fingerprints, and TLS.',
        icon: Layers,
        accentColor: 'indigo',
        accentBorder: 'border-indigo-500/40 hover:border-indigo-500/60',
        accentBg: 'bg-indigo-500/10',
        accentText: 'text-indigo-400',
        badgeClass: 'bg-indigo-950/70 border-indigo-500/40 text-indigo-300',
        stripeGradient: 'from-indigo-500/40 via-indigo-500/10 to-transparent',
    },
    {
        id: 'threat',
        name: 'Threat, Blacklist & Cyber Risk (ZoomEye.ai)',
        shortName: 'Threat & Risk',
        description: 'ZoomEye cyberspace telemetry, multi-vendor reputation, and CVE vulnerability flags.',
        icon: ShieldAlert,
        accentColor: 'rose',
        accentBorder: 'border-rose-500/40 hover:border-rose-500/60',
        accentBg: 'bg-rose-500/10',
        accentText: 'text-rose-400',
        badgeClass: 'bg-rose-950/70 border-rose-500/40 text-rose-300',
        stripeGradient: 'from-rose-500/40 via-rose-500/10 to-transparent',
    },
    {
        id: 'dns',
        name: 'DNS & Nameserver Infrastructure',
        shortName: 'DNS Infrastructure',
        description: 'Authoritative nameservers, MX mail clusters, TXT identity verification, and SOA records.',
        icon: Network,
        accentColor: 'amber',
        accentBorder: 'border-amber-500/40 hover:border-amber-500/60',
        accentBg: 'bg-amber-500/10',
        accentText: 'text-amber-400',
        badgeClass: 'bg-amber-950/70 border-amber-500/40 text-amber-300',
        stripeGradient: 'from-amber-500/40 via-amber-500/10 to-transparent',
    },
];

const collapsedBranches = ref({});

const toggleBranchCollapse = (branchId) => {
    collapsedBranches.value[branchId] = !collapsedBranches.value[branchId];
};

// Grouped Discovered Findings by Branch for Bento Layout
const groupedDiscoveredFindings = computed(() => {
    let sourceList = discoveredFindings.value;
    if (showHitsOnly.value) {
        sourceList = sourceList.filter(f => f.status === 'found');
    }

    const groups = branchCategoryConfig.map(branch => {
        const findings = sourceList.filter(f => f.category === branch.id);
        const verifiedHits = findings.filter(f => f.status === 'found').length;
        return {
            ...branch,
            findings,
            totalCount: findings.length,
            verifiedHits,
            isClean: findings.length === 0 || (branch.id === 'threat' && verifiedHits === 0),
        };
    });

    const knownIds = new Set(branchCategoryConfig.map(b => b.id));
    const extraFindings = sourceList.filter(f => !f.category || !knownIds.has(f.category));
    if (extraFindings.length > 0) {
        groups.push({
            id: 'other',
            name: 'Supplementary Intelligence Vectors',
            shortName: 'Supplementary',
            description: 'Additional passive telemetry, headers, and OSINT correlation findings.',
            icon: Compass,
            accentColor: 'indigo',
            accentBorder: 'border-indigo-500/40 hover:border-indigo-500/60',
            accentBg: 'bg-indigo-500/10',
            accentText: 'text-indigo-400',
            badgeClass: 'bg-indigo-950/70 border-indigo-500/40 text-indigo-300',
            stripeGradient: 'from-indigo-500/40 via-indigo-500/10 to-transparent',
            findings: extraFindings,
            totalCount: extraFindings.length,
            verifiedHits: extraFindings.filter(f => f.status === 'found').length,
            isClean: false,
        });
    }

    if (selectedCategory.value !== 'all') {
        return groups.filter(g => g.id === selectedCategory.value);
    }

    return groups;
});

const scrollToBranch = (branchId) => {
    if (branchId === 'all') {
        selectedCategory.value = 'all';
        return;
    }
    selectedCategory.value = 'all';
    nextTick(() => {
        const el = document.getElementById(`branch-${branchId}`);
        if (el) {
            el.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    });
};

// Filtered Catalog Tools for Sidebar
const filteredCatalogTools = computed(() => {
    const q = toolSearchQuery.value.trim().toLowerCase();
    let tools = activeBranch.value === 'all'
        ? allToolsList.value
        : (categorizedTools.value[activeBranch.value] || []);

    if (q) {
        tools = tools.filter(t =>
            t.name.toLowerCase().includes(q) ||
            t.description.toLowerCase().includes(q) ||
            t.tag.toLowerCase().includes(q)
        );
    }

    return tools;
});

// Helper for finding icon / status color
const getStatusBadgeClass = (status) => {
    switch (status) {
        case 'found':
            return 'bg-teal-950/80 text-teal-300 border border-teal-700/60 font-bold';
        case 'clean':
            return 'bg-cyan-950/70 text-cyan-300 border border-cyan-800/50';
        case 'info':
            return 'bg-slate-900 text-slate-300 border border-slate-700/60';
        default:
            return 'bg-slate-900 text-slate-400 border border-slate-800';
    }
};

// Clipboard Helpers
const copyText = (text, id = 'field') => {
    if (!text) return;
    navigator.clipboard.writeText(text);
    copiedField.value = id;
    setTimeout(() => {
        if (copiedField.value === id) copiedField.value = null;
    }, 2000);
};

const copyUrl = (url, id) => {
    if (!url) return;
    navigator.clipboard.writeText(url);
    copiedUrlId.value = id;
    setTimeout(() => {
        if (copiedUrlId.value === id) copiedUrlId.value = null;
    }, 2000);
};

// Start Real-Time SSE Probing Stream
const startLiveStream = (target) => {
    stopLiveStream();
    if (!target) return;

    isStreaming.value = true;
    allStreamedResults.value = [];
    streamProgress.value = {
        probed: 0,
        total: 7,
        found: discoveredFindings.value.filter(f => f.status === 'found').length,
        percent: 0,
        last_tool: 'Initializing Domain Scanner...',
        last_status: '',
        duration_ms: 0,
    };

    const streamUrl = `/api/osint/domain/stream?target=${encodeURIComponent(target)}`;
    eventSource = new EventSource(streamUrl);

    eventSource.addEventListener('start', (event) => {
        try {
            const data = JSON.parse(event.data);
            streamProgress.value.total = data.total_tools || 7;
        } catch (e) {
            console.error('Failed to parse SSE start event', e);
        }
    });

    eventSource.addEventListener('result', (event) => {
        try {
            const result = JSON.parse(event.data);
            allStreamedResults.value.push(result);

            const exists = discoveredFindings.value.some(f => f.id === result.tool_id);
            if (!exists) {
                discoveredFindings.value.push({
                    id: result.tool_id,
                    platform: result.platform || result.tool_name,
                    website_domain: result.website_domain || 'external',
                    url: result.url,
                    category: result.category || 'dns',
                    category_label: result.category_label || result.branch_label,
                    status: result.status,
                    summary: result.summary,
                    data: result.data || {},
                });
            }
        } catch (e) {
            console.error('Failed to parse SSE result event', e);
        }
    });

    eventSource.addEventListener('progress', (event) => {
        try {
            const progress = JSON.parse(event.data);
            streamProgress.value.probed = progress.probed;
            streamProgress.value.total = progress.total;
            streamProgress.value.found = discoveredFindings.value.filter(f => f.status === 'found').length;
            streamProgress.value.percent = progress.percent;
            streamProgress.value.last_tool = progress.current_tool;
            streamProgress.value.last_status = progress.branch;
        } catch (e) {
            console.error('Failed to parse SSE progress event', e);
        }
    });

    eventSource.addEventListener('done', (event) => {
        try {
            const summary = JSON.parse(event.data);
            streamProgress.value.duration_ms = summary.duration_ms;
            streamProgress.value.percent = 100;
        } catch (e) {
            console.error('Failed to parse SSE done event', e);
        } finally {
            stopLiveStream();
        }
    });

    eventSource.addEventListener('error', () => {
        stopLiveStream();
    });
};

const stopLiveStream = () => {
    if (eventSource) {
        eventSource.close();
        eventSource = null;
    }
    isStreaming.value = false;
};

// Execute Target Probe & Start SSE Stream
const executeLookup = async (targetParam = null) => {
    const target = (targetParam || targetInput.value || '').trim();
    if (!target) return;

    targetInput.value = target;
    currentTarget.value = target;
    isProbing.value = true;

    try {
        const response = await axios.post('/api/osint/domain/probe', { target });
        probeData.value = response.data;
        discoveredFindings.value = response.data.initial_findings ? [...response.data.initial_findings] : [];

        // Update URL query state
        const currentParams = new URLSearchParams(window.location.search);
        currentParams.set('target', target);
        window.history.replaceState({}, '', `${window.location.pathname}?${currentParams.toString()}`);

        // Start SSE stream
        startLiveStream(target);
    } catch (err) {
        console.error('Error probing domain target:', err);
    } finally {
        isProbing.value = false;
    }
};

const setExampleTarget = (val) => {
    targetInput.value = val;
    executeLookup(val);
};

// Bookmark Modal
const openBookmarkModal = (item) => {
    openBookmark({
        title: `OSINT [${item.category_label || 'Domain'}]: ${item.platform || item.name} (${currentTarget.value})`,
        url: item.url,
        notes: item.summary || item.description || '',
        severity: item.status === 'found' ? 'high' : 'medium',
        investigation_id: props.investigations?.length ? props.investigations[0].id : null,
    });
};

// Export Intelligence Report
const exportReport = (format) => {
    const target = currentTarget.value || 'domain-target';
    const telemetry = probeData.value?.telemetry || {};
    showExportMenu.value = false;

    if (format === 'json') {
        const payload = {
            target: currentTarget.value,
            telemetry: telemetry,
            discovered_findings: discoveredFindings.value,
            exported_at: new Date().toISOString(),
        };
        downloadJson(payload, `domain-osint-${target}.json`);
    } else if (format === 'markdown') {
        let md = `# OSINT Domain Intelligence Report: ${target}\n`;
        md += `*Exported on ${new Date().toUTCString()}*\n\n`;
        md += `## Target Telemetry\n`;
        md += `- **Primary IP:** ${telemetry.primary_ip || 'N/A'}\n`;
        md += `- **Registrar:** ${telemetry.whois?.registrar || 'N/A'}\n`;
        md += `- **Created:** ${telemetry.whois?.created_at || 'N/A'}\n`;
        md += `- **Nameservers:** ${(telemetry.nameservers || []).join(', ') || 'N/A'}\n\n`;
        md += `## Discovered Intelligence Findings (${discoveredFindings.value.length})\n\n`;

        discoveredFindings.value.forEach((f, idx) => {
            md += `### ${idx + 1}. [${f.category_label || f.category}] ${f.platform}\n`;
            md += `- **Status:** ${f.status.toUpperCase()}\n`;
            md += `- **URL:** ${f.url}\n`;
            md += `- **Summary:** ${f.summary}\n\n`;
        });

        downloadMarkdown(md, `domain-osint-${target}.md`);
    }
};
</script>

<template>
    <div class="space-y-6 w-full">
        <!-- Top Navigation Breadcrumb & Actions -->
        <div class="border-b border-slate-800/80 pb-4">
            <div class="flex items-center gap-2 text-xs font-mono text-slate-500 mb-2">
                <Link href="/osint" class="hover:text-teal-400 transition flex items-center gap-1">
                    <Compass class="w-3.5 h-3.5" />
                    <span>OSINT Framework</span>
                </Link>
                <span>/</span>
                <span class="text-teal-400/90 font-semibold">Domain Name</span>
                <span class="px-2 py-0.5 text-[10px] bg-teal-500/10 border border-teal-500/20 text-teal-400 rounded-full font-bold uppercase tracking-wider">
                    Active Engines
                </span>
            </div>

            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl font-serif font-black tracking-tight text-white uppercase flex items-center gap-2.5">
                        <Globe class="w-6 h-6 text-teal-400" />
                            <span>Domain Lookup &amp; Recon Discovery</span>
                        </h1>
                        <p class="text-slate-400 text-xs font-mono mt-0.5">
                            Automated discovery across Shodan, urlscan.io, ZoomEye.ai, crt.sh, and DNS zone authority.
                        </p>
                    </div>

                    <div class="flex items-center gap-2.5 font-mono text-xs">
                        <div v-if="currentTarget" class="px-2.5 py-1 rounded-lg bg-slate-900/80 border border-slate-800 flex items-center gap-2">
                            <span class="text-slate-500">Target:</span>
                            <span class="text-slate-200 font-bold">{{ currentTarget }}</span>
                        </div>

                        <div v-if="currentTarget" class="px-2.5 py-1 rounded-lg bg-teal-950/40 border border-teal-800/50 text-teal-300 font-bold flex items-center gap-1.5">
                            <ShieldCheck class="w-3.5 h-3.5 text-teal-400" />
                            <span>{{ discoveredFindings.length }} Findings</span>
                        </div>

                        <!-- Export Dropdown -->
                        <div v-if="currentTarget" class="relative">
                            <button
                                type="button"
                                @click="showExportMenu = !showExportMenu"
                                class="px-2.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white flex items-center gap-1.5 transition text-xs"
                            >
                                <Download class="w-3.5 h-3.5 text-teal-400" />
                                <span>Export</span>
                                <ChevronDown class="w-3 h-3 text-slate-500" />
                            </button>

                            <div
                                v-if="showExportMenu"
                                class="absolute right-0 mt-2 w-48 bg-slate-900/95 border border-slate-800 rounded-xl shadow-2xl py-1 z-50 font-mono text-xs backdrop-blur-md"
                            >
                                <button
                                    type="button"
                                    @click="exportReport('markdown')"
                                    class="w-full text-left px-3.5 py-2 hover:bg-slate-800 text-slate-300 hover:text-teal-300 flex items-center gap-2 transition"
                                >
                                    <FileText class="w-3.5 h-3.5 text-teal-400" />
                                    <span>Export Markdown (.md)</span>
                                </button>
                                <button
                                    type="button"
                                    @click="exportReport('json')"
                                    class="w-full text-left px-3.5 py-2 hover:bg-slate-800 text-slate-300 hover:text-teal-300 flex items-center gap-2 transition"
                                >
                                    <FileText class="w-3.5 h-3.5 text-cyan-400" />
                                    <span>Export JSON (.json)</span>
                                </button>
                            </div>
                        </div>

                        <Link
                            href="/osint"
                            class="px-3 py-1 rounded bg-slate-900/90 border border-slate-800 hover:border-teal-500/40 text-slate-400 hover:text-teal-300 transition-colors"
                        >
                            &larr; Hub
                        </Link>
                    </div>
                </div>
            </div>

            <!-- Search Input Console -->
            <div class="p-3 sm:p-4 rounded-xl bg-slate-900/70 border border-slate-800 shadow-xl">
                <form @submit.prevent="executeLookup()" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
                    <div class="relative flex-1">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
                            <Search class="w-4 h-4 text-teal-400" />
                        </div>
                        <input
                            v-model="targetInput"
                            type="text"
                            placeholder="Enter domain name or URL (e.g. github.com, cloudflare.com, proton.me)..."
                            class="w-full pl-10 pr-24 py-2 rounded-lg bg-slate-950 border border-slate-800 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-teal-500/60 focus:ring-1 focus:ring-teal-500/30 text-xs font-mono transition"
                            required
                        />
                        <!-- Engine Indicators -->
                        <div class="absolute inset-y-0 right-0 pr-2.5 flex items-center pointer-events-none gap-1 hidden sm:flex">
                            <span class="px-1.5 py-0.5 text-[9px] font-mono rounded bg-slate-900 border border-slate-800 text-slate-400">Shodan</span>
                            <span class="px-1.5 py-0.5 text-[9px] font-mono rounded bg-slate-900 border border-slate-800 text-slate-400">urlscan</span>
                            <span class="px-1.5 py-0.5 text-[9px] font-mono rounded bg-slate-900 border border-slate-800 text-slate-400">ZoomEye</span>
                        </div>
                    </div>

                    <!-- Action Buttons -->
                    <div class="flex items-center gap-2">
                        <button
                            type="submit"
                            :disabled="isProbing || !targetInput.trim()"
                            class="px-5 py-2 rounded-lg bg-teal-500 hover:bg-teal-400 disabled:opacity-50 text-slate-950 font-mono text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 shadow transition-all shrink-0"
                        >
                            <Loader2 v-if="isProbing" class="w-3.5 h-3.5 animate-spin" />
                            <Activity v-else class="w-3.5 h-3.5" />
                            <span>Domain Recon</span>
                        </button>

                        <button
                            v-if="isStreaming"
                            type="button"
                            @click="stopLiveStream()"
                            class="px-3 py-2 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-300 font-mono text-xs rounded-lg transition-colors flex items-center gap-1.5 shrink-0"
                            title="Stop Live Stream"
                        >
                            <StopCircle class="w-3.5 h-3.5" />
                            <span>Halt</span>
                        </button>
                    </div>
                </form>

                <!-- Example Targets & Auto-Detection -->
                <div class="flex flex-wrap items-center justify-between gap-2 pt-2 text-[11px] font-mono">
                    <div class="flex flex-wrap items-center gap-1.5">
                        <span class="text-slate-500">Quick Targets:</span>
                        <button
                            type="button"
                            @click="setExampleTarget('github.com')"
                            class="px-2 py-0.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-teal-400 rounded transition-colors"
                        >
                            github.com
                        </button>
                        <button
                            type="button"
                            @click="setExampleTarget('cloudflare.com')"
                            class="px-2 py-0.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-teal-400 rounded transition-colors"
                        >
                            cloudflare.com
                        </button>
                        <button
                            type="button"
                            @click="setExampleTarget('shodan.io')"
                            class="px-2 py-0.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-cyan-400 rounded transition-colors"
                        >
                            shodan.io
                        </button>
                        <button
                            type="button"
                            @click="setExampleTarget('zoomeye.ai')"
                            class="px-2 py-0.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-rose-400 rounded transition-colors"
                        >
                            zoomeye.ai
                        </button>
                    </div>

                    <div class="text-slate-500 hidden md:block">
                        Engines: Shodan InternetDB • urlscan.io Public • ZoomEye Matrix • crt.sh CT
                    </div>
                </div>
            </div>

            <!-- ========================================================================= -->
            <!-- 4 TARGET TELEMETRY HUD CARDS                                              -->
            <!-- ========================================================================= -->
            <div v-if="probeData?.telemetry" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 font-mono">
                <!-- Card 1: Domain Target & Apex Info -->
                <div class="bg-slate-900/80 border border-slate-800 rounded-xl p-3.5 space-y-2">
                    <div class="flex items-center justify-between text-slate-400">
                        <span class="text-[10px] uppercase tracking-wider text-slate-500">Domain Target</span>
                        <Globe class="w-3.5 h-3.5 text-teal-400" />
                    </div>

                    <div class="flex items-center justify-between">
                        <span class="text-sm font-bold text-white truncate select-all">{{ probeData.telemetry.domain }}</span>
                        <button
                            type="button"
                            @click="copyText(probeData.telemetry.domain, 'domain_name')"
                            class="text-slate-500 hover:text-teal-400 transition-colors"
                            title="Copy Domain Name"
                        >
                            <Check v-if="copiedField === 'domain_name'" class="w-3.5 h-3.5 text-emerald-400" />
                            <Copy v-else class="w-3.5 h-3.5" />
                        </button>
                    </div>

                    <div class="border-t border-slate-800/80 pt-1.5 flex items-center justify-between text-[11px] text-slate-400">
                        <span>Status:</span>
                        <span class="text-teal-300 truncate max-w-[180px]">{{ probeData.telemetry.whois?.status || 'Active / Delegated' }}</span>
                    </div>
                </div>

                <!-- Card 2: Primary IPv4 & GeoIP -->
                <div class="bg-slate-900/80 border border-slate-800 rounded-xl p-3.5 space-y-2">
                    <div class="flex items-center justify-between text-slate-400">
                        <span class="text-[10px] uppercase tracking-wider text-slate-500">Primary IP &amp; Geo</span>
                        <MapPin class="w-3.5 h-3.5 text-cyan-400" />
                    </div>

                    <div class="text-sm font-bold text-white truncate flex items-center gap-1.5">
                        <span class="text-cyan-300 font-bold select-all">{{ probeData.telemetry.primary_ip || 'No A Record' }}</span>
                        <span v-if="probeData.telemetry.geoip?.country_code" class="text-xs text-slate-400 font-normal">
                            ({{ probeData.telemetry.geoip.country_code }})
                        </span>
                    </div>

                    <div class="border-t border-slate-800/80 pt-1.5 flex items-center justify-between text-[11px] text-slate-400">
                        <span class="truncate">{{ probeData.telemetry.geoip?.city || 'Location' }}, {{ probeData.telemetry.geoip?.country || 'Global' }}</span>
                        <a
                            v-if="probeData.telemetry.primary_ip"
                            :href="`/osint/ip?target=${probeData.telemetry.primary_ip}`"
                            class="text-cyan-400 hover:text-cyan-300 shrink-0 ml-1 flex items-center gap-0.5 text-[10px]"
                            title="Pivot to IP Tool"
                        >
                            <span>Pivot IP</span>
                            <ArrowRight class="w-2.5 h-2.5" />
                        </a>
                    </div>
                </div>

                <!-- Card 3: Authoritative Nameservers & MX -->
                <div class="bg-slate-900/80 border border-slate-800 rounded-xl p-3.5 space-y-2">
                    <div class="flex items-center justify-between text-slate-400">
                        <span class="text-[10px] uppercase tracking-wider text-slate-500">DNS &amp; Mail Infrastructure</span>
                        <Network class="w-3.5 h-3.5 text-amber-400" />
                    </div>

                    <div class="text-sm font-bold text-white truncate" :title="(probeData.telemetry.nameservers || []).join(', ')">
                        {{ (probeData.telemetry.nameservers || [])[0] || 'Unknown NS' }}
                    </div>

                    <div class="border-t border-slate-800/80 pt-1.5 flex items-center justify-between text-[11px] text-slate-400">
                        <span>NS: {{ (probeData.telemetry.nameservers || []).length }} Servers</span>
                        <span class="text-amber-300">MX: {{ (probeData.telemetry.mail_servers || []).length }} Mail</span>
                    </div>
                </div>

                <!-- Card 4: Registrar WHOIS & DNSSEC -->
                <div class="bg-slate-900/80 border border-slate-800 rounded-xl p-3.5 space-y-2">
                    <div class="flex items-center justify-between text-slate-400">
                        <span class="text-[10px] uppercase tracking-wider text-slate-500">Registrar &amp; DNSSEC</span>
                        <ShieldCheck class="w-3.5 h-3.5 text-emerald-400" />
                    </div>

                    <div class="text-sm font-bold text-white truncate" :title="probeData.telemetry.whois?.registrar">
                        {{ probeData.telemetry.whois?.registrar || 'Accredited Registrar' }}
                    </div>

                    <div class="border-t border-slate-800/80 pt-1.5 flex items-center justify-between text-[11px] text-slate-400">
                        <span>Created: {{ probeData.telemetry.whois?.created_at || 'Active' }}</span>
                        <span class="text-emerald-400 font-bold">{{ probeData.telemetry.whois?.dnssec || 'DNSSEC' }}</span>
                    </div>
                </div>
            </div>

            <!-- ========================================================================= -->
            <!-- DUAL-COLUMN RECONNAISSANCE WORKSPACE                                       -->
            <!-- ========================================================================= -->
            <div class="flex flex-col lg:flex-row items-start gap-6">

                <!-- ===================================================================== -->
                <!-- LEFT COLUMN: MANUAL FRAMEWORK CATALOG (Sidebar Panel ~340px)          -->
                <!-- ===================================================================== -->
                <aside class="w-full lg:w-80 xl:w-96 shrink-0 space-y-4 lg:sticky lg:top-4">
                    <div class="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl space-y-4">
                        <div class="flex items-center justify-between pb-3 border-b border-slate-800">
                            <div class="flex items-center gap-2">
                                <Wrench class="w-4 h-4 text-teal-400" />
                                <h2 class="text-xs font-mono font-bold text-slate-200 uppercase tracking-wider">
                                    Domain Tools Directory
                                </h2>
                            </div>
                            <span class="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700/50">
                                {{ allToolsList.length }} Tools
                            </span>
                        </div>

                        <!-- Search Catalog Input -->
                        <div class="relative">
                            <input
                                v-model="toolSearchQuery"
                                type="text"
                                placeholder="Filter tools, tags, or engines..."
                                class="w-full pl-8 pr-3 py-1.5 rounded-lg bg-slate-950 border border-slate-800 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-teal-500/60 text-xs font-mono transition"
                            />
                            <Search class="w-3.5 h-3.5 text-slate-500 absolute left-2.5 top-2.5 pointer-events-none" />
                        </div>

                        <!-- Branch Filter Dropdown / Tabs -->
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-mono uppercase tracking-wider text-slate-500 block">
                                Branch Directory:
                            </label>
                            <select
                                v-model="activeBranch"
                                class="w-full py-1.5 px-2.5 rounded-lg bg-slate-950 border border-slate-800 text-slate-200 text-xs font-mono focus:outline-none focus:border-teal-500/60 cursor-pointer"
                            >
                                <option value="all">All Branches ({{ allToolsList.length }})</option>
                                <option
                                    v-for="branch in branchDefinitions"
                                    :key="branch.id"
                                    :value="branch.id"
                                >
                                    {{ branch.name }} ({{ branch.count }})
                                </option>
                            </select>
                        </div>

                        <!-- Tools Scrollable List with Hidden Scrollbar -->
                        <div class="space-y-2 max-h-[calc(100vh-320px)] overflow-y-auto no-scrollbar">
                            <div
                                v-for="tool in filteredCatalogTools"
                                :key="tool.id"
                                class="p-2.5 rounded-xl bg-slate-950/80 hover:bg-slate-900 border border-slate-800 hover:border-slate-700 transition space-y-1.5 group"
                            >
                                <div class="flex items-start justify-between gap-2">
                                    <div class="min-w-0">
                                        <div class="text-xs font-mono font-bold text-slate-200 group-hover:text-teal-400 transition-colors truncate">
                                            {{ tool.name }}
                                        </div>
                                        <span class="text-[9px] font-mono px-1.5 py-0.2 rounded bg-slate-900 text-teal-400/90 border border-slate-800">
                                            {{ tool.tag }}
                                        </span>
                                    </div>

                                    <div class="flex items-center gap-1 shrink-0">
                                        <button
                                            type="button"
                                            @click="openBookmarkModal(tool)"
                                            class="p-1 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-teal-300 transition"
                                            title="Bookmark tool to Dossier"
                                        >
                                            <FolderPlus class="w-3 h-3" />
                                        </button>

                                        <a
                                            :href="tool.url"
                                            target="_blank"
                                            rel="noreferrer"
                                            class="p-1 rounded bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/30 text-teal-400 hover:text-teal-300 transition"
                                            title="Launch Tool Directly"
                                        >
                                            <ExternalLink class="w-3 h-3" />
                                        </a>
                                    </div>
                                </div>

                                <p class="text-[11px] font-mono text-slate-400 leading-snug line-clamp-2">
                                    {{ tool.description }}
                                </p>

                                <div class="pt-1 flex items-center justify-between font-mono text-[10px] text-slate-500">
                                    <span class="truncate max-w-[180px]">{{ tool.url.replace(/^https?:\/\//, '') }}</span>
                                    <button
                                        type="button"
                                        @click="copyUrl(tool.url, tool.id)"
                                        class="px-1.5 py-0.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-slate-200 flex items-center gap-1 text-[9px]"
                                    >
                                        <Check v-if="copiedUrlId === tool.id" class="w-2.5 h-2.5 text-teal-400" />
                                        <Copy v-else class="w-2.5 h-2.5" />
                                        <span>{{ copiedUrlId === tool.id ? 'Copied' : 'Copy' }}</span>
                                    </button>
                                </div>
                            </div>

                            <div v-if="filteredCatalogTools.length === 0" class="p-6 text-center text-slate-500 text-xs">
                                No domain tools matched filter.
                            </div>
                        </div>
                    </div>
                </aside>

                <!-- ===================================================================== -->
                <!-- RIGHT COLUMN: DISCOVERED INTELLIGENCE FINDINGS (Main Feed)            -->
                <!-- ===================================================================== -->
                <main class="flex-1 min-w-0 space-y-5">
                    <!-- Awaiting Target Empty State -->
                    <div
                        v-if="!currentTarget && !isProbing"
                        class="p-12 rounded-xl bg-slate-900/60 border border-slate-800/80 text-center space-y-3"
                    >
                        <div class="w-14 h-14 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-500 mx-auto">
                            <Compass class="w-7 h-7 text-teal-400/80" />
                        </div>
                        <h3 class="text-base font-serif font-bold text-slate-200 uppercase tracking-tight">
                            Awaiting Domain Target
                        </h3>
                        <p class="text-xs font-mono text-slate-400 max-w-md mx-auto leading-relaxed">
                            Enter an apex domain name or URL in the console above to initiate automated multi-branch reconnaissance across Shodan, urlscan.io, ZoomEye.ai, crt.sh, and DNS records.
                        </p>
                    </div>

                    <!-- Loading State -->
                    <div
                        v-if="isProbing && !discoveredFindings.length && !isStreaming"
                        class="p-12 rounded-xl bg-slate-900/70 border border-slate-800 text-center space-y-4 font-mono"
                    >
                        <Loader2 class="w-8 h-8 text-teal-400 animate-spin mx-auto" />
                        <div class="text-sm text-slate-200 font-bold uppercase tracking-wide">
                            Probing Global Domain Infrastructure &amp; Engines...
                        </div>
                        <div class="text-xs text-slate-500">
                            Querying Shodan InternetDB, urlscan.io scanner, ZoomEye Matrix, and Certificate Transparency logs
                        </div>
                    </div>

                    <!-- 1. LIVE TELEMETRY CONTROL BAR SECTION -->
                    <section
                        v-if="currentTarget && (isStreaming || discoveredFindings.length > 0)"
                        class="p-4 sm:p-5 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl space-y-4"
                    >
                        <!-- Section Header -->
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-slate-800/80">
                            <div class="flex items-center gap-2.5">
                                <div class="w-7 h-7 rounded-lg bg-teal-950/50 border border-teal-800/50 flex items-center justify-center text-teal-400 shrink-0">
                                    <Activity class="w-4 h-4" :class="{ 'animate-pulse text-teal-300': isStreaming }" />
                                </div>
                                <div>
                                    <div class="flex items-center gap-2">
                                        <h2 class="text-xs font-mono font-bold text-slate-200 uppercase tracking-wider">
                                            Domain Discovery Telemetry
                                        </h2>
                                        <span v-if="isStreaming" class="flex h-2 w-2 relative" title="Live reconnaissance stream active">
                                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-teal-400 opacity-75"></span>
                                            <span class="relative inline-flex rounded-full h-2 w-2 bg-teal-500"></span>
                                        </span>
                                        <span v-else class="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-800/80 text-slate-400 border border-slate-700/50">
                                            Scan Complete
                                        </span>
                                    </div>
                                    <p class="text-[10px] font-mono text-slate-400 mt-0.5">
                                        Confirmed findings and intelligence extracted from Shodan, urlscan.io, ZoomEye, and DNS zone
                                    </p>
                                </div>
                            </div>

                            <div class="flex items-center gap-2 self-end sm:self-auto font-mono text-xs">
                                <div class="px-2.5 py-1 rounded-lg bg-teal-950/40 border border-teal-800/50 text-teal-300 font-bold flex items-center gap-1.5">
                                    <CheckCircle2 class="w-3.5 h-3.5 text-teal-400" />
                                    <span>{{ discoveredFindings.length }} Findings</span>
                                </div>

                                <button
                                    v-if="isStreaming"
                                    type="button"
                                    @click="stopLiveStream()"
                                    class="px-2.5 py-1 rounded-lg bg-rose-950/40 hover:bg-rose-900/50 border border-rose-800/60 text-rose-300 flex items-center gap-1.5 transition text-xs"
                                >
                                    <StopCircle class="w-3.5 h-3.5" />
                                    <span>Stop</span>
                                </button>
                                <button
                                    v-else
                                    type="button"
                                    @click="startLiveStream(currentTarget)"
                                    class="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 hover:text-white flex items-center gap-1.5 transition text-xs"
                                >
                                    <RefreshCw class="w-3.5 h-3.5" />
                                    <span>Re-scan</span>
                                </button>
                            </div>
                        </div>

                        <!-- Live Telemetry Progress Bar -->
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between text-[11px] font-mono text-slate-400">
                                <div class="flex items-center gap-2">
                                    <span class="text-slate-300 font-semibold">
                                        {{ streamProgress.percent === 100 ? 'Recon Scan Complete' : `Probing ${streamProgress.total} engines & vectors...` }}
                                    </span>
                                    <span class="font-bold" :class="streamProgress.percent === 100 ? 'text-emerald-400' : 'text-teal-400'">
                                        [{{ streamProgress.probed }}/{{ streamProgress.total }}]
                                    </span>
                                    <span class="text-slate-500">—</span>
                                    <span class="text-slate-400">{{ discoveredFindings.length }} Findings Logged</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <span v-if="isStreaming && streamProgress.last_tool" class="text-slate-500 truncate max-w-[180px]">
                                        Engine: {{ streamProgress.last_tool }}
                                    </span>
                                    <span
                                        class="font-bold px-1.5 py-0.5 rounded text-[10px] transition-all duration-300"
                                        :class="streamProgress.percent === 100
                                            ? 'bg-emerald-950/70 border border-emerald-500/40 text-emerald-300 shadow-[0_0_8px_rgba(52,211,153,0.3)]'
                                            : 'text-teal-400'"
                                    >
                                        {{ streamProgress.percent }}%
                                    </span>
                                </div>
                            </div>

                            <!-- Outer Track Container with Glow & Border Glow -->
                            <div
                                class="h-2 w-full bg-slate-950 rounded-full overflow-hidden border relative transition-all duration-500"
                                :class="streamProgress.percent === 100
                                    ? 'animate-complete-glow'
                                    : (isStreaming ? 'border-teal-500/40 shadow-[0_0_10px_rgba(45,212,191,0.2)]' : 'border-slate-800')"
                            >
                                <!-- Inner Progress Fill with Dynamic Gradient, Tactical Stripes & Glistening Effects -->
                                <div
                                    class="h-full rounded-full relative transition-all duration-300 overflow-hidden"
                                    :class="streamProgress.percent === 100
                                        ? 'animate-complete-flow'
                                        : 'bg-gradient-to-r from-teal-500 via-cyan-400 to-indigo-500'"
                                    :style="{ width: `${streamProgress.percent}%` }"
                                >
                                    <!-- 1. Active Tactical Moving Stripes (While Loading) -->
                                    <div
                                        v-if="isStreaming"
                                        class="absolute inset-0 animate-tactical-stripes opacity-40"
                                    ></div>

                                    <!-- 2. Traveling Laser Highlight Beam (While Loading) -->
                                    <div
                                        v-if="isStreaming"
                                        class="absolute inset-0 w-24 bg-gradient-to-r from-transparent via-white/50 to-transparent animate-scan-laser"
                                    ></div>

                                    <!-- 3. Continuous Micro-Drift Tactical Pattern (When 100% Finished) -->
                                    <div
                                        v-if="streamProgress.percent === 100"
                                        class="absolute inset-0 animate-complete-drift pointer-events-none"
                                    ></div>

                                    <!-- 4. Glistening Crystalline Sheen Sweep from Left to Right (When 100% Finished) -->
                                    <div
                                        v-if="streamProgress.percent === 100"
                                        class="absolute inset-0 w-full animate-glisten-sweep pointer-events-none"
                                    ></div>
                                </div>

                                <!-- 5. Pulsing Leading-Edge Scanner Tip (While Loading) -->
                                <div
                                    v-if="isStreaming && streamProgress.percent > 0 && streamProgress.percent < 100"
                                    class="absolute top-0 bottom-0 w-2.5 bg-cyan-200 rounded-full animate-pulse-tip pointer-events-none -ml-1.5"
                                    :style="{ left: `${streamProgress.percent}%` }"
                                ></div>
                            </div>
                        </div>

                        <!-- 5 Category Filter Tabs & Verified Hits Toggle -->
                        <div class="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-slate-800/60">
                            <!-- Category Pills -->
                            <div class="flex flex-wrap items-center gap-1.5 font-mono text-[11px]">
                                <button
                                    v-for="cat in categoriesList"
                                    :key="cat.id"
                                    type="button"
                                    @click="scrollToBranch(cat.id)"
                                    class="px-2.5 py-1 rounded-md transition flex items-center gap-1.5 border cursor-pointer"
                                    :class="selectedCategory === cat.id
                                        ? 'bg-teal-950/60 border-teal-500/50 text-teal-300 font-bold shadow-sm'
                                        : 'bg-slate-950/60 hover:bg-slate-900 border-slate-800/80 text-slate-400 hover:text-slate-200'"
                                    :title="cat.id === 'all' ? 'View all branches' : `Glide down to ${cat.label}`"
                                >
                                    <span>{{ cat.label }}</span>
                                    <span class="px-1 py-0.2 rounded text-[10px]" :class="selectedCategory === cat.id ? 'bg-teal-800/40 text-teal-200' : 'bg-slate-900 text-slate-500'">
                                        {{ categoryCounts[cat.id] || 0 }}
                                    </span>
                                </button>
                            </div>

                            <!-- Verified Hits Only Checkbox -->
                            <label class="flex items-center gap-1.5 text-[11px] font-mono text-slate-400 cursor-pointer select-none">
                                <input
                                    type="checkbox"
                                    v-model="showHitsOnly"
                                    class="rounded bg-slate-950 border-slate-800 text-teal-500 focus:ring-teal-500/30 w-3.5 h-3.5"
                                />
                                <span>Show verified hits only</span>
                            </label>
                        </div>
                    </section>

                    <!-- ============================================================= -->
                    <!-- BENTO BRANCH FINDINGS CONTAINERS (Separated Window per Branch)-->
                    <!-- ============================================================= -->
                    <template v-if="currentTarget && (isStreaming || discoveredFindings.length > 0)">
                        <template v-if="discoveredFindings.length > 0">
                            <!-- Loop over each Branch Bento Box Container as a Separated Window -->
                            <div
                                v-for="branch in groupedDiscoveredFindings"
                                :key="branch.id"
                                :id="`branch-${branch.id}`"
                                class="rounded-2xl bg-slate-900/40 border border-slate-800/90 shadow-xl p-4 sm:p-5 space-y-4 backdrop-blur-sm relative overflow-hidden transition-all scroll-mt-20 group/bento"
                                :class="branch.accentBorder"
                            >
                                <!-- Top Accent Indicator Gradient Stripe -->
                                <div
                                    class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r"
                                    :class="branch.stripeGradient"
                                ></div>

                                <!-- Bento Container Header Bar -->
                                <div class="flex items-center justify-between gap-3 pb-3 border-b border-slate-800/70">
                                    <div class="flex items-center space-x-3 min-w-0">
                                        <div
                                            class="w-8 h-8 rounded-xl border flex items-center justify-center shrink-0 shadow-sm"
                                            :class="[branch.accentBg, branch.accentBorder]"
                                        >
                                            <component :is="branch.icon" class="w-4 h-4" :class="branch.accentText" />
                                        </div>
                                        <div class="min-w-0">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <h3 class="font-mono font-bold text-xs uppercase tracking-wider text-slate-100">
                                                    {{ branch.name }}
                                                </h3>
                                                <span
                                                    class="px-2 py-0.5 text-[10px] font-mono rounded-full font-bold uppercase tracking-wider border shadow-sm"
                                                    :class="branch.badgeClass"
                                                >
                                                    {{ branch.verifiedHits }} Verified
                                                </span>
                                            </div>
                                            <p class="text-[11px] font-mono text-slate-400 mt-0.5 truncate">
                                                {{ branch.description }}
                                            </p>
                                        </div>
                                    </div>

                                    <div class="flex items-center gap-2 shrink-0">
                                        <button
                                            type="button"
                                            @click="toggleBranchCollapse(branch.id)"
                                            class="p-1.5 rounded-lg bg-slate-950/60 hover:bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-200 transition-colors"
                                            :title="collapsedBranches[branch.id] ? 'Expand Branch Window' : 'Collapse Branch Window'"
                                        >
                                            <ChevronDown
                                                class="w-4 h-4 transition-transform duration-200"
                                                :class="{ '-rotate-90': collapsedBranches[branch.id] }"
                                            />
                                        </button>
                                    </div>
                                </div>

                                <!-- Collapsed Summary Preview -->
                                <div
                                    v-if="collapsedBranches[branch.id]"
                                    class="py-2 px-3 rounded-xl bg-slate-950/40 border border-slate-800/50 flex items-center justify-between text-xs font-mono text-slate-400"
                                >
                                    <span>{{ branch.findings.length }} items detected (Window collapsed)</span>
                                    <button
                                        type="button"
                                        @click="toggleBranchCollapse(branch.id)"
                                        class="text-teal-400 hover:underline"
                                    >
                                        Expand details
                                    </button>
                                </div>

                                <!-- Expanded Bento Findings Content -->
                                <div v-else class="space-y-3.5">
                                    <!-- Clean State Card if zero detections -->
                                    <div
                                        v-if="branch.findings.length === 0"
                                        class="p-6 rounded-xl bg-slate-950/50 border border-slate-800/60 text-center space-y-2 font-mono"
                                    >
                                        <ShieldCheck class="w-6 h-6 text-emerald-400/80 mx-auto" />
                                        <div class="text-xs text-slate-300 font-bold uppercase tracking-wider">
                                            Clean Baseline State
                                        </div>
                                        <p class="text-[11px] text-slate-500 max-w-sm mx-auto">
                                            Zero anomalous or risk indicators detected in this branch for {{ currentTarget }}.
                                        </p>
                                    </div>

                                    <!-- Grid of Finding Items for this Branch -->
                                    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                                        <div
                                            v-for="item in branch.findings"
                                            :key="item.id"
                                            class="p-3.5 rounded-xl bg-slate-950/90 border border-slate-800/90 hover:border-slate-700/80 transition shadow-sm space-y-2.5 flex flex-col justify-between group/card relative"
                                        >
                                            <!-- Card Header -->
                                            <div class="flex items-start justify-between gap-2">
                                                <div class="flex items-center gap-2 min-w-0">
                                                    <span
                                                        class="px-1.5 py-0.5 text-[9px] font-mono uppercase tracking-wider rounded border"
                                                        :class="getStatusBadgeClass(item.status)"
                                                    >
                                                        {{ item.status }}
                                                    </span>
                                                    <h4 class="text-xs font-mono font-bold text-slate-200 truncate group-hover/card:text-teal-400 transition-colors">
                                                        {{ item.platform }}
                                                    </h4>
                                                </div>

                                                <!-- Action Buttons: Bookmark & External Launch -->
                                                <div class="flex items-center gap-1 shrink-0">
                                                    <button
                                                        type="button"
                                                        @click="openBookmarkModal(item)"
                                                        class="p-1 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-teal-300 transition"
                                                        title="Pin to Case Dossier"
                                                    >
                                                        <FolderPlus class="w-3 h-3" />
                                                    </button>
                                                    <a
                                                        :href="item.url"
                                                        target="_blank"
                                                        rel="noreferrer"
                                                        class="px-2 py-1 rounded bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/30 text-teal-400 hover:text-teal-300 transition flex items-center gap-1 text-[10px] font-mono font-semibold"
                                                        title="Launch External Engine / Pivot"
                                                    >
                                                        <span>Launch</span>
                                                        <ExternalLink class="w-2.5 h-2.5" />
                                                    </a>
                                                </div>
                                            </div>

                                            <!-- Summary & Evidence -->
                                            <p class="text-[11px] font-mono text-slate-300 leading-relaxed">
                                                {{ item.summary }}
                                            </p>

                                            <!-- Structured Data Tags -->
                                            <div v-if="item.data && Object.keys(item.data).length > 0" class="pt-1 flex flex-wrap gap-1">
                                                <!-- Open Ports preview -->
                                                <template v-if="item.data.ports && item.data.ports.length">
                                                    <span
                                                        v-for="port in item.data.ports.slice(0, 8)"
                                                        :key="port"
                                                        class="px-1.5 py-0.5 text-[9px] font-mono rounded bg-cyan-950/60 border border-cyan-800/40 text-cyan-300"
                                                    >
                                                        :{{ port }}
                                                    </span>
                                                    <span v-if="item.data.ports.length > 8" class="text-[9px] text-slate-500 font-mono py-0.5">
                                                        +{{ item.data.ports.length - 8 }} more
                                                    </span>
                                                </template>

                                                <!-- Subdomains preview -->
                                                <template v-else-if="item.data.subdomains && item.data.subdomains.length">
                                                    <span
                                                        v-for="sub in item.data.subdomains.slice(0, 4)"
                                                        :key="sub"
                                                        class="px-1.5 py-0.5 text-[9px] font-mono rounded bg-teal-950/60 border border-teal-800/40 text-teal-300 truncate max-w-[140px]"
                                                        :title="sub"
                                                    >
                                                        {{ sub }}
                                                    </span>
                                                    <span v-if="item.data.subdomains.length > 4" class="text-[9px] text-slate-500 font-mono py-0.5">
                                                        +{{ item.data.subdomains.length - 4 }} subdomains
                                                    </span>
                                                </template>

                                                <!-- Nameservers preview -->
                                                <template v-else-if="item.data.nameservers && item.data.nameservers.length">
                                                    <span
                                                        v-for="ns in item.data.nameservers.slice(0, 2)"
                                                        :key="ns"
                                                        class="px-1.5 py-0.5 text-[9px] font-mono rounded bg-amber-950/60 border border-amber-800/40 text-amber-300 truncate max-w-[160px]"
                                                        :title="ns"
                                                    >
                                                        {{ ns }}
                                                    </span>
                                                </template>

                                                <!-- Web Server / Header preview -->
                                                <template v-else-if="item.data.server">
                                                    <span class="px-1.5 py-0.5 text-[9px] font-mono rounded bg-indigo-950/60 border border-indigo-800/40 text-indigo-300">
                                                        Server: {{ item.data.server }}
                                                    </span>
                                                </template>
                                            </div>

                                            <!-- Card Footer URL & Copy -->
                                            <div class="border-t border-slate-900 pt-2 flex items-center justify-between font-mono text-[10px] text-slate-500">
                                                <span class="truncate max-w-[200px]" :title="item.url">
                                                    {{ item.website_domain || 'external' }}
                                                </span>
                                                <button
                                                    type="button"
                                                    @click="copyUrl(item.url, item.id)"
                                                    class="text-slate-500 hover:text-slate-300 flex items-center gap-1 transition-colors"
                                                >
                                                    <Check v-if="copiedUrlId === item.id" class="w-3 h-3 text-teal-400" />
                                                    <Copy v-else class="w-3 h-3" />
                                                    <span>{{ copiedUrlId === item.id ? 'Copied' : 'Copy URL' }}</span>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </template>

                        <!-- Zero Findings Fallback -->
                        <div v-else class="p-8 rounded-2xl bg-slate-900/40 border border-slate-800 text-center space-y-2 font-mono">
                            <ShieldAlert class="w-8 h-8 text-amber-400/80 mx-auto" />
                            <div class="text-xs text-slate-300 font-bold uppercase tracking-wider">
                                No Target Findings Logged Yet
                            </div>
                            <p class="text-[11px] text-slate-500 max-w-sm mx-auto">
                                The automated scanner did not encounter indexed hostnames or exposed banners. Use the manual tools directory in the left sidebar to query external deep archives.
                            </p>
                        </div>
                    </template>
                </main>
            </div>

        <!-- Evidence Bookmark Modal -->
        <EvidenceBookmarkModal
            v-model="showBookmarkModal"
            :target="bookmarkTarget"
            :investigations="investigations"
            source="osint_domain_framework"
        />
    </div>
</template>
